...
class UserServiceImpl @Inject()(
  ...
  extends UserService with DAOSlick {
  ...
  override def save(user: User): Future[User] = {
    val dbUser = DBUser(None, user.firstName, user.lastName, user.email, user.avatarURL, Option(user.activated))
    val dbLoginInfo =
    ...
    val loginInfoAction = {
      ...
      for {
        ...
        loginInfo <- loginInfoOption
            .map(DBIO.successful(_)).getOrElse(insertLoginInfo)
      } yield loginInfo
    }
